...
class BtreePage { 
    ...
    static final int maxStrKeySize = (keySpace - strKeySize*2)/4;
    ...
    static boolean find(StorageImpl db, int pageId, Key firstKey, Key lastKey, 
                        int type, int height, ArrayList result)
    {
    ...
    static int clonePage(StorageImpl db, int origPageId, int type, int height) 
    {
        ...
        System.arraycopy(origPage.data, 0, newPage.data, 0, Page.pageSize); 

        if (--height != 0) {
            if (type == ClassDescriptor.tpString) { // page of strings
                for (i = 0; i <= n; i++) { 
                    setKeyStrOid(newPage, i, 
                                 clonePage(db, getKeyStrOid(origPage, i), type, height));
                }
            } else { 
                for (i = 0; i <= n; i++) {
                    setReference(newPage, maxItems-i-1, 
                                 clonePage(db, getReference(origPage, maxItems-i-1), type, height));
                }
            }
        }
    ...
    static int insert(StorageImpl db, int pageId, int type, BtreeKey ins, int height, 
                      boolean unique, boolean overwrite)
    {
        ...
        try { 
            if (type == ClassDescriptor.tpString) {         
                ...
                while (l < r)  {
                    int i = (l+r) >> 1;
                    if (compare(ins.key, pg, i) > 0) l = i+1; else r = i;
                }
                Assert.that(l == r);
                /* insert before e[r] */
                if (--height != 0) {
    ...
    static int compactify(Page pg, int m) 
    {
    ...
    static int remove(StorageImpl db, int pageId, int type, BtreeKey rem, int height)
    {
        Page pg = db.getPage(pageId);
        try { 
